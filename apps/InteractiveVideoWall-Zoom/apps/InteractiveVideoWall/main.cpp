...
int minDistance, maxDistance, middleDistance;	//Limits of depth distance
	...
	if (blobs.size() > 0)
	{
		...
		for (CvBlobs::const_iterator it = blobs.begin(); it != blobs.end(); ++it)
		{
			...
			if (largestBlobLabel == comparingBlobLabel)
			{
				...
				// Get depth value in the point of calculated Centroid
				//unsigned char *imageData = (unsigned char*)(finalDepthImage_gray.data);
				//valueCentroid = (int)imageData[finalDepthImage_gray.step * (int)XY.x + (int)XY.y];
				valueCentroid = finalDepthImage_gray.at<uchar>(int(P0),int(P1)); /*TODO*/
			}
		}	
	}
	else
...
//#endif
	...
	//createTrackbar("Centroid", main_window_name, &valueCentroid, 100); /*TODO*/
	...
	while (true)
	{
		...
		*/
		...
		minDistance = threshold_value;
		maxDistance = threshold_value2;
		middleDistance = (minDistance + maxDistance) / 2;

		if (valueCentroid >= middleDistance || valueCentroid < minDistance)
		{
			//cout << "Full --------------- ";
