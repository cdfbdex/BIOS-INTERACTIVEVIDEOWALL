...
list<TuioCursor*> stickyCursorList;			// Send position through cursor
list<TuioObject*> stickyObjectList;			// Send position and area through objects
...
// Depth distance variables
int depthDistance;

/*TODO*/
int screenResWidth = 0;
int screenResHeight = 0;
...
/*TODO*/
void MoveCursorToPosition(float xPos, float yPos, int maxHorizontal, int maxVertical)
{
	//POINT p;
	//GetCursorPos(&p);
	//int c = waitKey(1);;
	...
	//	p.x -= 10;  // Move Cursor Left
	//if (c == 'w') // Up Arrow Key
	//	p.y += 10;  // Move Cursor Up
	//if (c == 's') // Down Arrow KEy
	//	p.y -= 10;  // Move Cursor Down

	if (xPos > 0 && xPos < 1
		&& yPos > 0 && yPos < 1)
	{
		SetCursorPos(int(xPos*maxHorizontal), int(yPos*maxVertical));
	}
}
	...
	*/
	...
	if (blobs.size() > 0)
	{
		...
		for (CvBlobs::const_iterator it = blobs.begin(); it != blobs.end(); ++it)
		{
			...
			// Normalize data to send it via TUIO
			float X = XY.x;
			float Y = XY.y;

			/*TODO: Normalizar depthDistance*/
			depthDistance = finalDepthImage_gray.at<uchar>(int(X), int(Y)); /*TODO*/
			//float normalizedDepthDistance = (float)depthDistance / 255.0;
			float normalizedDepthDistance = 0.0;

			X = X / (DepthImageWidth*detectedDevices);
			Y = Y / (DepthImageHeight / ScaleHeight);

			if (it == blobs.begin())
			{
				//MoveCursorToPosition(X, depthDistance, screenResWidth, screenResHeight);
				MoveCursorToPosition(0.5, 0.5, screenResWidth, screenResHeight);
				cout << depthDistance << endl;
			}



			// SEND DATA THROUGH CURSOR CLASS
			std::list<TuioCursor*> cursorList = tuio->getTuioCursors();

			bool existe_blob = false;
			TuioCursor *user = NULL;
			for (std::list<TuioCursor*>::iterator tuioCursor = cursorList.begin(); tuioCursor != cursorList.end(); tuioCursor++)
			{
				if ((*tuioCursor)->getCursorID() == comparingBlobLabel)
				{
					user = (*tuioCursor);
					existe_blob = true;
					break;
			...
			{
				tuio->addTuioCursor(X, normalizedDepthDistance, 0);
			}
			else
			{
				tuio->updateTuioCursor(user, X, normalizedDepthDistance, 0);
				
			}

			// SEND DATA THROUGH OBJECT CLASS
			std::list<TuioObject*> objectList = tuio->getTuioObjects();

			bool existe_blob2 = false;
			TuioObject *user2 = NULL;
			for (std::list<TuioObject*>::iterator tuioObject = objectList.begin(); tuioObject != objectList.end(); tuioObject++)
			{
				if ((*tuioObject)->getSymbolID() == comparingBlobLabel)
				{
					user2 = (*tuioObject);
					existe_blob2 = true;
					break;
			...
			if (existe_blob2 == false)
			{
				tuio->addTuioObject(comparingBlobLabel, X, normalizedDepthDistance, 0.5);
			}
			else
			{
				tuio->updateTuioObject(user2, X, normalizedDepthDistance, 1.5);
			}
	...
	// Zoom frame from video
	//Linear equation to crop video_frame from 1/3 to 1 of its original size
	/*
	Calculate the origin, length side of the rectangle that crop frame_video to make zoom.
	threshold_value and threshold_value2 define the depth distance in which person is detected, its values are from 0 to 255,
	depending on the trackbars value set in the interface. Zoom starts when person is placed on the first half of valid depth distance.
	When person is far and when is near, zoom origin goes from 0 to 1/3 and length side from 1 to 1/3 respectively.
	*/

	int minDistance = threshold_value;
	int maxDistance = threshold_value2;
	int middleDistance = (minDistance + maxDistance) / 2;

	if (depthDistance >= middleDistance || depthDistance < minDistance)
	{
		//cout << "Full --------------- ";
	...
	else
	{
		...
		//cout << "Zoom--- ";
		//int frameRows = frame_video.rows;
		//int frameCols = frame_video.cols;

		//float origin = float((-0.33333 / (middleDistance - minDistance)) * (valueCentroid - middleDistance));
		//float sideLength = float((0.66666 / (middleDistance - minDistance)) * (valueCentroid - middleDistance) + 1.0);

		//Mat cropped_frame_video(frame_video, Rect(int(origin*frameCols), int(origin*frameRows), int(sideLength*frameCols), int(sideLength*frameRows)));				// Size of zoom

		//cout << "minDist: " << threshold_value << " maxDist: " << threshold_value2 << " middleDist: " << middleDistance << endl;
		//cout << "origin: " << origin << " sideLength: " << sideLength;
		//imshow(video_window_name, cropped_frame_video);
	}
...
//#endif
	...
	/*TODO*/
	GetDesktopResolution(screenResWidth, screenResHeight);
