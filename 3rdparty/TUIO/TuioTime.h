...
namespace TUIO {
	...
	private:
		long long seconds, micro_seconds;
		static long long start_seconds, start_micro_seconds, Processor_Frecuency;
	...
	public:
		...
		//static LARGE_INTEGER Tick_Counter;		
		 ...
		 */	
		TuioTime (long long sec, long long usec) {
			seconds = sec;
			micro_seconds = usec;
		};
		 ...
		 */	
		TuioTime operator+(long long us) {
			long long sec = seconds + us/USEC_SECOND;
			long long usec = micro_seconds + us%USEC_SECOND;
		...
		TuioTime operator+(TuioTime ttime) {
			long long sec = seconds + ttime.getSeconds();
			long long usec = micro_seconds + ttime.getMicroseconds();
		...
		bool operator==(TuioTime ttime) {
			if ((seconds==(long long)ttime.getSeconds()) && (micro_seconds==(long long)ttime.getMicroseconds())) return true;
			else return false;
		};
		...
		//static void getFrecuencyProcessor() {
		//	QueryPerformanceFrequency(&Processor_Frequency);
		//};

		//static void getTickCounter() {
		//	QueryPerformanceCounter(&Tick_Counter);
		//};
